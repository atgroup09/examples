...
# Select latest data from 'ts_delta_view3'

# Example - select all rows:
# python.exe 008-select-from-view3.py

# Example - select rows by tag-name 'A1':
# python.exe 008-select-from-view3.py A1
...
if len(sys.argv) < 2:
    ...
    cursor.execute("SELECT * FROM my_table_view3");
else:
    # select data from the database-table by a tag-name
    cursor.execute("SELECT * FROM my_table_view3 WHERE tag=?", (sys.argv[1],));
